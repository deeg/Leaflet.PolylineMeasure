(function (factory) {
	...
	L.Control.PolylineMeasure = L.Control.extend({
		...
		options: {
			...
			position: 'topleft',
			/**
			 * Which units the distances are displayed in. Possible values are: 'meter', 'landmile', 'nauticalmile'
			 * @type {String}
			 * @default
			 */
			unit: 'meter',
		...
		onAdd: function(map) {
			...
			var title = self.options.measureControlTitle ? self.options.measureControlTitle : 'Polyline Measure ' + ((self.options.unit === 'nauticalmile') ? '[nautical miles]' : ((self.options.unit === 'landmile') ? '[land miles]' : '[meters]'));
		...
		_toggleMeasure: function () {
			...
			//	self._polylineArc (L.latLng (50, 1), L.latLng(20, 100)).addTo(self._layerPaint);
		...
		_getDistance: function (distance) {
			...
			var dist = distance;
			var symbol;
			if (self.options.unit === 'nauticalmile') {
				unit = "nm";
				if (dist >= 1852000) {
			...
			} else if (self.options.unit === 'landmile') {
				unit = "mi";
				if (dist >= 1609344) {
		...
		_polylineArc: function (_from, _to) {
			    // work with copy of _from-Object otherwise it would get modiefied due to call-by-reference on Objects in Javascript
		...
		/*			
				...
				if ( from.lng*180/Math.PI < -180) {
					
						lngInterpol = lngInterpol  -360;
					} else if  ( from.lng*180/Math.PI > 180) {
						lngInterpol = lngInterpol  +360;
					} else {
					  // from.lng within -180° to +180°					
					}
				*/	
		...
		_mouseClick: function(e) {
			...
			if(self._fixedLine) {
				...
				self._fixedLine.setLatLngs (self._fixedLine.getLatLngs().concat (self._polylineArc (self._currentCircleCoords, e.latlng)));
